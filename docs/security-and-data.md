...
Referral codes identify a referrer but grant no authorization. Patient and provider registration may capture a code only at account creation; email, phone, URL type, or an existing account cannot be used for retroactive claiming. Referral history omits health events and account/contact details. Points are awarded only by server-side completion/approval events, stored in an append-only ledger, and cannot be supplied by public/admin request DTOs. Provider URL intent is checked against authoritative provider classification.
