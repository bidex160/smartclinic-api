...
New booking creation performs a read-only eligibility selection first so the booking can snapshot one ProviderService's exact fulfilment price before funding. Verified payment still moves the booking to `PENDING_PROVIDER_MATCH` and triggers the offer workflow after settlement commits; matching never starts from `DRAFT` or `AWAITING_FUNDING`, and it is restricted to the commercial binding. Provider acceptance records the assignment-level `ACCEPTED` event and automatically proceeds to `CONFIRMED` and `SCHEDULED`. If the bound Provider is no longer eligible, the booking becomes `UNFULFILLABLE`; it is never silently switched to another price/provider.
