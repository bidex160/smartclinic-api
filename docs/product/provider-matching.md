...
SmartClinic supports two provider onboarding paths. `POST /api/v1/admin/providers` captures the operational provider identity and email, creates a `PENDING`/`INVITED` Provider plus initial invitation, and attempts email delivery without a second invitation form. `POST /api/v1/public/providers/register` creates a provider-only User, credential, linked Provider, and `PENDING`/`SUBMITTED` application. Invitation acceptance similarly links the new account and submits a complete invited profile but never activates it.

Providers inspect/update their permitted profile at `/api/v1/provider/profile` and submit or resubmit through `/api/v1/provider/onboarding/submit`. ADMIN/OPERATIONS review through `/api/v1/admin/providers/:id/approve` or `/reject`; approval is the only first-activation path and requires a complete submitted profile plus an active linked User carrying `PROVIDER`. Rejection preserves Provider, User, role, and configuration. Capability, location, availability, and exception management remains administrative for now.
