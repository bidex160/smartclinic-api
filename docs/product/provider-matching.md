...
`OFFERED`, `DECLINED`, and `EXPIRED` assignments hold no capacity. `RELEASED` and `CANCELLED` reservations no longer block eligibility. A service-level release operation exists for a future booking cancellation or accepted-assignment withdrawal workflow; this task does not expose a direct reservation mutation API or implement cancellation policy. V1 has no automatic HELD timeout separate from the accepted-assignment lifecycle.
