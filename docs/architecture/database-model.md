...
Referral rewards use separate persistence boundaries. `referral_codes` provides one immutable public code per User. `referrals` records one direct registration relationship and its authoritative target/lifecycle. `reward_rules` configures future qualification awards; `reward_points_ledger` is append-only and uniquely keyed by business event. `reward_level_definitions`, `reward_level_requirements`, and `reward_level_achievements` define and persist Level 1 without hardcoded progress conditions. `reward_conversion_rates` configures integer-safe points-to-currency conversion.
