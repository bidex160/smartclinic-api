...
Provider self-service configuration continues to use these existing Provider-owned tables; no ownership or duplicate schema was introduced. A derived onboarding-readiness projection counts active capabilities, active linked physical locations, and active weekly availability. It is evaluated at submission and approval time rather than persisted. Availability exceptions are optional, and HOME_VISIT service-area readiness remains deferred.

Provider onboarding adds normalized operational `email`, optional `phone`, `provider_type`, `country_code`, `state_or_region`, `city`, and review fields. `onboarding_status` (`DRAFT`, `INVITED`, `SUBMITTED`, `APPROVED`, `REJECTED`) is intentionally distinct from operational Provider status. Existing ACTIVE providers are backfilled to `APPROVED` so deployed eligible providers remain eligible; their existing linked User email is copied where available. Other legacy providers remain `DRAFT` until explicitly completed and submitted. New application DTOs require the operational identity fields even though selected database columns remain nullable for safe legacy preservation.
...
Referral rewards use separate persistence boundaries. `referral_codes` provides one immutable public code per User. `referrals` records one direct registration relationship and its authoritative target/lifecycle. `reward_rules` configures qualification and optional level-completion awards; `reward_points_ledger` is append-only and uniquely keyed by business event. `reward_level_definitions`, `reward_level_requirements`, and `reward_level_achievements` define cumulative Levels 1–5 with explicit ordinals and one requirement per level/target. The same aggregate direct-qualified counts drive every level; no referral-tree persistence exists. `reward_conversion_rates` configures integer-safe points-to-currency conversion.
...
Health Check `provider_services` are already unique per Provider, package, and fulfilment mode, so they store that exact offering's required `price_minor/currency` pair directly. `bookings.commercial_provider_id/commercial_provider_service_id` preserve the pre-funding binding, while existing `quoted_amount/currency` remain the immutable financial snapshot consumed by funding and payment verification. Legacy global `package_prices` are not a live commercial authority.
...
Find Care adds immutable `providers.provider_reference`, controlled `care_service_definitions`, and unique Provider-owned `provider_care_services`. General care services deliberately remain separate from the package/mode-specific Health Check `provider_services` capability table. `provider_care_service_delivery_options` uniquely pairs each offering and delivery mode with an explicit non-negative minor-unit price and currency. `care_requests.service_price_minor/service_currency` form a nullable pair only until a no-preference request is assigned; thereafter they are the historical commercial snapshot.
...
`platform_commission_settings` is a singleton configuration row whose nullable Provider commission rate represents an explicitly unconfigured platform. `providers.commission_override_bps` is nullable so inheritance differs from an explicit zero-percent agreement. Both use constrained integer basis points from 0 through 10,000. `commission_config_history` is append-only and records target, Provider where applicable, old/new rates, actor, and timestamp. These tables configure future Provider-revenue snapshots only; they do not modify current patient-facing prices or payment transactions.

`provider_earnings` is the immutable allocation snapshot for Provider-revenue products. It stores gross, commission basis points/source/amount, Provider share, currency, Provider, product source, and optional collection transaction in integer minor units. Unique source and partial payment-transaction indexes enforce exactly-once creation. Only status and payable/settled timestamps evolve; append-only `provider_earning_status_history` records transitions. Balances are grouped ledger aggregates and are never merged across currencies.
