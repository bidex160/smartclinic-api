...
The matching workflow is automatic and sequential: after verified payment settlement commits, eligibility discovery supplies the first candidate and creates one offer. Operations confirms accepted assignments and performs audited retry/manual/reassignment intervention; authenticated providers list/respond only to assignments resolved through their `User → Provider` link. Assignment and booking transitions are transactional and append their respective history records. Automated ranking, final confirmation, notifications, and scheduled expiry execution remain outside this foundation.
