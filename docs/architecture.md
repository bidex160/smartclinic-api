...
The Providers module owns provider service capabilities, their fulfilment-mode-specific Health Check prices, physical locations, recurring weekly availability (including an optional exclusive booking-start cutoff), one-off scheduling exceptions, booking-derived provider capacity reservations, eligibility discovery, and provider-offer/assignment state. Eligibility derives appointment end from Health Check package duration, applies date-specific additions/removals after the weekly start-time baseline, and excludes overlapping held/confirmed work. Reservation creation/promotion is transactional with assignment acceptance/confirmation. The Health Checks module owns package clinical/catalogue metadata and fulfilment-mode definitions, but it is no longer the patient-facing commercial price authority.
...
Provider revenue accounting is separate from payment collection. `PaymentTransaction` proves SmartClinic collected money; `ProviderEarning` snapshots the Provider allocation for an eligible product. Health Check settlement creates the earning atomically as `HELD`, and authoritative encounter completion advances it to `PAYABLE`. Settlement remains a separate future domain.
