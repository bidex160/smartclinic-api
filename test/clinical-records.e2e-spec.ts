...
import { MeClinicalRecordAccessController, ProviderClinicalRecordAccessRequestsController, ProviderSharedClinicalRecordsController, ProviderSharedHealthPassportsController } from '../src/clinical-records/clinical-record-access.controller';
...
describe('Clinical Records authorization (e2e)', () => {
  ...
  it('exposes Shareable Health Passport only through provider authority', async()=>{
    const route='/api/v1/provider/shared-health-passports/SCP-AB12-CD34';
    await request(app.getHttpServer()).get(route).expect(401);await request(app.getHttpServer()).get(route).set('Authorization','Bearer patient').expect(403);await request(app.getHttpServer()).get(route).set('Authorization','Bearer provider').expect(200);
    expect(access.getSharedHealthPassport).toHaveBeenCalledWith(expect.objectContaining({id:'provider-user'}),'SCP-AB12-CD34');
  });
});
