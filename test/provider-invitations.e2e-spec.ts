...
describe('Provider invitation HTTP boundaries (e2e)', () => {
  ...
  it('allows ADMIN creation and returns the token only on creation', () => request(app.getHttpServer()).post(adminPath).set('Authorization', 'Bearer admin').send({ email: 'provider@example.test' }).expect(201).expect((response) => { expect(response.body.invitationToken).toBe(token); expect(response.body).not.toHaveProperty('tokenHash'); }));
