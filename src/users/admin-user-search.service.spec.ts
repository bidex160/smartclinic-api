...
describe('AdminUserSearchService', () => {
  ...
  it('excludes soft-deleted users explicitly', async () => { await subject.search({ q: 'ada', page: 1, limit: 20 }); expect(builder.withDeleted).toHaveBeenCalled(); expect(builder.where).toHaveBeenCalledWith('user.deleted_at IS NULL'); });
