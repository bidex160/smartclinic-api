...
export class AdminUserSearchService {
  ...
  async search(query: AdminUserSearchQueryDto): Promise<AdminUserSearchResponseDto> {
    ...
    const builder = this.users.createQueryBuilder('user').withDeleted().leftJoinAndSelect('user.provider', 'provider')
      ...
      .andWhere('(user.email_normalized ILIKE :contains OR user.display_name ILIKE :contains)', { contains })
      .orderBy('CASE WHEN user.email_normalized = :exact THEN 0 WHEN user.email_normalized LIKE :prefix OR LOWER(user.display_name) LIKE :prefix THEN 1 ELSE 2 END', 'ASC')
      .addOrderBy('user.email_normalized', 'ASC', 'NULLS LAST').addOrderBy('user.display_name', 'ASC', 'NULLS LAST').addOrderBy('user.id', 'ASC')
