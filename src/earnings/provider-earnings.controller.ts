...
export class ProviderEarningsController {
  ...
  @Get('summary') @ApiOperation({ summary: 'Get own ledger-derived balances by currency' }) @ApiOkResponse({ type: ProviderEarningBalanceDto, isArray: true }) summary(@Req() request: { user: User }) { return this.earnings.balancesOwn(request.user); }
...
@ApiTags('Admin Provider earnings') @ApiBearerAuth() @UseGuards(JwtAuthGuard, RolesGuard) @Roles(UserRole.ADMIN, UserRole.OPERATIONS) @Controller('admin/provider-earnings')
export class AdminProviderEarningsController {
  constructor(private readonly earnings: ProviderEarningsService) {}
  @Get('summary') @ApiOperation({ summary: 'Get Provider earning balances by currency' }) @ApiOkResponse({ type: ProviderEarningBalanceDto, isArray: true }) summary(@Query() query: AdminProviderEarningBalanceQueryDto) { return this.earnings.balancesAdmin(query.providerId); }
  @Get() @ApiOperation({ summary: 'List Provider earnings across Providers' }) @ApiOkResponse({ type: ProviderEarningListResponseDto }) list(@Query() query: AdminProviderEarningListQueryDto) { return this.earnings.listAdmin(query); }
