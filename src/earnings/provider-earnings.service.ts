...
export class ProviderEarningsService {
  ...
  async releaseMaturedCompletedWalletEarnings(manager?:EntityManager){const m=manager??this.earnings.manager;const rows=await m.getRepository(ProviderEarning).createQueryBuilder('e').where('e.status=:status',{status:ProviderEarningStatus.HELD}).andWhere('e.paymentTransactionId IS NULL').andWhere('e.payableAt IS NOT NULL AND e.payableAt<=:now',{now:new Date()}).andWhere('e.sourceType IN (:...types)',{types:[ProviderEarningSourceType.PHARMACY_FULFILLMENT,ProviderEarningSourceType.DIAGNOSTIC_FULFILLMENT]}).setLock(manager?'pessimistic_write':undefined as any).getMany();let released=0;for(const earning of rows){earning.status=ProviderEarningStatus.PAYABLE;await m.getRepository(ProviderEarning).save(earning);await m.getRepository(ProviderEarningStatusHistory).save({providerEarningId:earning.id,fromStatus:ProviderEarningStatus.HELD,toStatus:ProviderEarningStatus.PAYABLE,actorUserId:null,reasonCode:'WALLET_SETTLEMENT_HOLD_MATURED',reasonNote:null});released++;}return{released};}
