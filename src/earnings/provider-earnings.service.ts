...
export class ProviderEarningsService {
  ...
  async createHeldPharmacyFulfillmentEarning(manager:EntityManager,input:{providerId:string;fulfillmentReference:string;grossAmountMinor:string;currency:string;paymentTransaction:PaymentTransaction}){const repository=manager.getRepository(ProviderEarning);const sourceType=ProviderEarningSourceType.PHARMACY_FULFILLMENT;const existing=await repository.findOne({where:{sourceType,sourceReference:input.fulfillmentReference},lock:{mode:'pessimistic_write'}});if(existing){if(existing.paymentTransactionId!==input.paymentTransaction.id)throw new ConflictException('Pharmacy earning belongs to another payment transaction');return existing;}if(input.paymentTransaction.status!==PaymentTransactionStatus.SUCCEEDED||input.paymentTransaction.transactionType!==PaymentTransactionType.COLLECTION||input.paymentTransaction.currency!==input.currency||this.toMinor(input.paymentTransaction.amount)!==BigInt(input.grossAmountMinor))throw new ConflictException('Payment transaction does not match pharmacy funding');const resolution=await this.commissions.requireForProvider(input.providerId,manager);const c=calculateCommission(BigInt(input.grossAmountMinor),resolution.rateBasisPoints);const earning=await repository.save(repository.create({providerId:input.providerId,paymentTransactionId:input.paymentTransaction.id,sourceType,sourceReference:input.fulfillmentReference,currency:input.currency,grossAmountMinor:input.grossAmountMinor,commissionBps:resolution.rateBasisPoints,commissionSource:resolution.source,commissionAmountMinor:c.commissionAmountMinor.toString(),providerShareMinor:c.providerShareMinor.toString(),status:ProviderEarningStatus.HELD,payableAt:null,settledAt:null}));await manager.getRepository(ProviderEarningStatusHistory).save({providerEarningId:earning.id,fromStatus:null,toStatus:ProviderEarningStatus.HELD,actorUserId:null,reasonCode:'PHARMACY_PAYMENT_SETTLED',reasonNote:null});return earning;}
