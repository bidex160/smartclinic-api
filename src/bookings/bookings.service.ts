...
export class BookingsService {
  constructor(
    ...
    @InjectRepository(HealthCheckConfigurationQuote)
    private readonly configurationQuotes: Repository<HealthCheckConfigurationQuote>,
  ...
  async createSelf(
    ...
    if (dto.configurationReference)
      return this.createFromQuote(user, patient, dto);
    if (!dto.healthCheckPackageId || !dto.fulfilmentModeId)
      throw new BadRequestException(
        "Legacy booking requires healthCheckPackageId and fulfilmentModeId, or supply configurationReference",
      );
    return this.create({
      ...dto,
  ...
  private async createFromQuote(
    ...
    for (
      let attempt = 0;
      attempt < MAX_BOOKING_REFERENCE_GENERATION_ATTEMPTS;
      attempt += 1
    ) {
      try {
        const saved = await this.bookingRepository.manager.transaction(
          async (manager) => {
            const quoteRepository = manager.getRepository(
            ...
            if (!quote)
              throw new NotFoundException(
                "Health Check configuration quote not found",
              );
            if (quote.bookingId) {
              const existing = await manager
                .getRepository(Booking)
                .findOne({ where: { id: quote.bookingId } });
              if (existing) return existing;
            }
            if (quote.consumedAt)
              throw new ConflictException(
                "Health Check configuration quote has already been consumed",
              );
            if (quote.expiresAt.getTime() <= Date.now())
              throw new ConflictException(
                "Health Check configuration quote has expired",
              );

            const offering = await manager
              .getRepository(ProviderService)
              .findOne({
            ...
            if (
              !offering?.isActive ||
              !offering.healthCheckPackage.isActive ||
              !offering.fulfilmentMode.isActive
            ) {
              throw new ConflictException(
                "Quoted Health Check offering is no longer available",
              );
            }
            this.validateVisitAddress(
              offering.fulfilmentMode.code,
              dto.visitAddress,
            );
            const end = offering.healthCheckPackage.estimatedDurationMinutes
              ? deriveAppointmentEndTime(
                  dto.preferredTimeWindowStart,
            ...
            const address =
              offering.fulfilmentMode.code === "HOME_VISIT" && dto.visitAddress
                ? this.normalizedAddress(dto.visitAddress)
                : null;
            const eligible =
              await this.providerCapabilities.findEligibleProviders(
            ...
            if (!eligible.some((candidate) => candidate.id === offering.id)) {
              throw new ConflictException(
                "Selected Provider is unavailable for the requested appointment",
              );
            }
            ...
            if (
              ...
              const addressRepository =
                manager.getRepository(BookingVisitAddress);
              await addressRepository.save(
                addressRepository.create({
                  ...this.normalizedAddress(dto.visitAddress),
                  latitude: this.normalizedAddress(dto.visitAddress).latitude ? String(this.normalizedAddress(dto.visitAddress).latitude) : null,
                  longitude: this.normalizedAddress(dto.visitAddress).longitude ? String(this.normalizedAddress(dto.visitAddress).longitude) : null,
                  bookingId: row.id,
                }),
              );
            }
        ...
        if (
          !isBookingReferenceCollision(error) ||
          attempt === MAX_BOOKING_REFERENCE_GENERATION_ATTEMPTS - 1
        )
          throw error;
      }
    }
  ...
  async create(
    ...
    const fulfilmentMode = await this.fulfilmentModeRepository.findOne({
      where: { id: createBookingDto.fulfilmentModeId },
    });
    this.validateVisitAddress(
      fulfilmentMode?.code ?? "",
      createBookingDto.visitAddress,
    );
    const capability = await this.selectCommercialCapability(
      createBookingDto,
      fulfilmentMode?.code ?? "",
    );
        ...
        const booking = await this.bookingRepository.manager.transaction(
          async (manager) => {
            ...
            const providerServiceRepository =
              manager.getRepository(ProviderService);
            const lockedProviderService =
              await providerServiceRepository.findOne({
                where: {
                  id: capability.id,
                  providerId: capability.providerId,
                  healthCheckPackageId: createBookingDto.healthCheckPackageId,
                  fulfilmentModeId: createBookingDto.fulfilmentModeId,
                  isActive: true,
                },
                lock: { mode: "pessimistic_read" },
              });
            if (!lockedProviderService)
              throw new ConflictException(
                "Selected provider Health Check offering is no longer available",
              );
            ...
            const packageConfiguration: Pick<
              HealthCheckPackage,
              "code" | "name" | "contents" | "addonAvailability"
            > = providerService.healthCheckPackage ?? {
              code: "",
              name: "",
              contents: [],
              addonAvailability: [],
            };
            const modeConfiguration: Pick<FulfilmentMode, "code" | "name"> =
              providerService.fulfilmentMode ?? { code: "", name: "" };
            const addonCodes = createBookingDto.addonCodes ?? [];
            if (addonCodes.includes("HOME_VISIT"))
              throw new BadRequestException(
                "HOME_VISIT is a fulfilment mode, not a clinical add-on",
              );
            const included = new Set(
              (packageConfiguration.contents ?? [])
                .filter((x) => x.isActive)
                .map((x) => x.clinicalContent.code),
            );
            const duplicateIncluded = addonCodes.find((code) =>
              included.has(code),
            );
            if (duplicateIncluded)
              throw new ConflictException(
                `Clinical add-on is already included in the package: ${duplicateIncluded}`,
              );
            const compatible = new Set(
              (packageConfiguration.addonAvailability ?? [])
                .filter((x) => x.isActive && x.clinicalContent.isActive)
                .map((x) => x.clinicalContent.code),
            );
            const capabilities = addonCodes.length
              ? await manager
                  .getRepository(ProviderServiceAddon)
                  .createQueryBuilder("capability")
                  .innerJoinAndSelect("capability.clinicalContent", "content")
                  .where("capability.providerServiceId=:serviceId", {
                    serviceId: providerService.id,
                  })
                  .andWhere("capability.isActive=true")
                  .andWhere("content.isActive=true")
                  .andWhere("content.code IN (:...codes)", {
                    codes: addonCodes,
                  })
            ...
            for (const code of addonCodes) {
              if (!compatible.has(code))
                throw new BadRequestException(
                  `Clinical add-on is unavailable for this package: ${code}`,
                );
              if (!capabilities.some((x) => x.clinicalContent.code === code))
                throw new ConflictException(
                  `Provider does not offer clinical add-on: ${code}`,
                );
            }
            if (
              capabilities.some((x) => x.currency !== providerService.currency)
            )
              throw new ConflictException(
                "Clinical add-on currency does not match package currency",
              );
            const addonTotal = capabilities.reduce(
              (sum, x) => sum + BigInt(x.priceMinor),
              0n,
            );
            const base = BigInt(providerService.priceMinor),
              fee = BigInt(providerService.fulfilmentFeeMinor ?? "0"),
              total = base + addonTotal + fee;
            const {
              visitAddress: _visitAddress,
              addonCodes: _addonCodes,
              ...bookingInput
            } = createBookingDto;
            const booking = bookingRepository.create({
              ...bookingInput,
              bookingReference: generateBookingReference(),
              organisationContextId:
                createBookingDto.organisationContextId ?? null,
              quotedAmount: this.fromMinor(total.toString()),
              currency: providerService.currency,
              basePackagePriceMinor: base.toString(),
              clinicalAddonsTotalMinor: addonTotal.toString(),
              fulfilmentFeeMinor: fee.toString(),
              commercialConfigurationSnapshot: {
                package: {
                  code: packageConfiguration.code,
                  name: packageConfiguration.name,
                },
                includedContents: (packageConfiguration.contents ?? [])
                  .filter((x) => x.isActive)
                  .sort((a, b) => a.sortOrder - b.sortOrder)
                  .map((x) => ({
                    code: x.clinicalContent.code,
                    name: x.clinicalContent.name,
                    category: x.clinicalContent.category,
                    resultType: x.clinicalContent.resultType,
                    unit: x.clinicalContent.unit,
                  })),
                selectedAddons: capabilities.map((x) => ({
                  code: x.clinicalContent.code,
                  name: x.clinicalContent.name,
                  category: x.clinicalContent.category,
                  resultType: x.clinicalContent.resultType,
                ...
                fulfilmentMode: {
                  code: modeConfiguration.code,
                  name: modeConfiguration.name,
                },
                pricing: {
                  basePackagePriceMinor: base.toString(),
                  clinicalAddonsTotalMinor: addonTotal.toString(),
              ...
              preferredTimeWindowStart:
                createBookingDto.preferredTimeWindowStart ?? null,
              preferredTimeWindowEnd: null,
              preferredTimezone: createBookingDto.preferredTimezone ?? null,
              preferredLocationNote:
                createBookingDto.preferredLocationNote ?? null,
              status: BookingStatus.DRAFT,
            });
            ...
            if (
              ...
              await addressRepository.save(
                addressRepository.create({
                  ...this.normalizedAddress(createBookingDto.visitAddress),
                   latitude: this.normalizedAddress(createBookingDto.visitAddress).latitude ? String(this.normalizedAddress(createBookingDto.visitAddress).latitude) : null,
                  longitude: this.normalizedAddress(createBookingDto.visitAddress).longitude ? String(this.normalizedAddress(createBookingDto.visitAddress).longitude) : null,
  ...
  private validateVisitAddress(
    modeCode: string,
    address: CreateBookingDto["visitAddress"],
  ): void {
    if (modeCode === "HOME_VISIT" && !address)
      throw new BadRequestException(
        "visitAddress is required for HOME_VISIT fulfilment",
      );
  }
  private normalizedAddress(
    value: NonNullable<CreateBookingDto["visitAddress"]>,
  ) {
    return {
      ...value,
  ...
  private async validateReferences(
    ...
    const [
      bookerExists,
      participantExists,
      healthCheckPackageExists,
      fulfilmentModeExists,
      organisationExists,
    ] = await Promise.all([
      this.userRepository.exists({
        where: { id: createBookingDto.bookerUserId },
      }),
      this.patientRepository.exists({
        where: { id: createBookingDto.participantPatientId },
      }),
      ...
      createBookingDto.organisationContextId
        ? this.organisationRepository.exists({
            where: { id: createBookingDto.organisationContextId },
          })
    ...
    if (
      ...
      throw new BadRequestException(
        "One or more booking references are invalid or unavailable",
      );
    }
  }

  private async selectCommercialCapability(
    dto: CreateBookingDto,
    modeCode: string,
  ) {
    const healthPackage = await this.healthCheckPackageRepository.findOne({
      where: { id: dto.healthCheckPackageId, isActive: true },
    });
    const end = healthPackage?.estimatedDurationMinutes
      ? deriveAppointmentEndTime(
          dto.preferredTimeWindowStart,
          healthPackage.estimatedDurationMinutes,
        )
      : null;
    if (!end)
      throw new BadRequestException(
    ...
    const address =
      modeCode === "HOME_VISIT" && dto.visitAddress
        ? this.normalizedAddress(dto.visitAddress)
        : null;
    const eligible = await this.providerCapabilities.findEligibleProviders(
      dto.healthCheckPackageId,
      dto.fulfilmentModeId,
      {
        requestedDate: dto.preferredDate,
        requestedStartTime: dto.preferredTimeWindowStart,
        requestedEndTime: end,
    ...
    if (!capability)
      throw new ConflictException(
        "No eligible priced Provider is currently available for this Health Check",
      );
    return capability;
  }
  private fromMinor(value: string) {
  ...
  private selfBookingNotFound(): never {
    throw new NotFoundException(
      "Health Check was not found for the authenticated patient",
    );
  }
  ...
  private async resolveSelfPatient(userId: string): Promise<Patient> {
    ...
    if (
      ...
      throw new NotFoundException(
        "SELF Patient identity was not found for the authenticated user",
      );
    return patient;
  }
}
