...
export class PublicBookingSessionService {
  ...
  async resolveBooking(token: string | null, reference: string): Promise<BookingResponseDto> { const session = await this.resolveSession(token, reference, true); return BookingResponseDto.fromEntity(session.booking); }
  async resolvePatientOwnershipProof(token: string | null, reference: string): Promise<string> { const session = await this.resolveSession(token, reference, false); return session.booking.participantPatientId; }
  ...
  private async resolveSession(token: string | null, reference: string, responseRelations: boolean): Promise<PublicBookingSession> {
    if (!token) this.deny();
    const relations = responseRelations ? { booking: { healthCheckPackage: true, fulfilmentMode: true, participant: true } } : { booking: true };
    const session = await this.sessions.findOne({ where: { tokenHash: this.hash(token!) }, relations });
    if (!session || session.revokedAt || session.expiresAt <= new Date() || session.booking.bookingReference !== reference) this.deny();
    session.lastUsedAt = new Date(); await this.sessions.save(session); return session;
  }
