...
export class CareAppointmentsService {
    ...
    for (let attempt = 0; attempt < MAX_CARE_APPOINTMENT_REFERENCE_ATTEMPTS; attempt += 1) {
      try {
        return await this.appointments.manager.transaction(async (manager) => {
          ...
          if (!offering || !offering.definition.isActive || !offering.supportsAppointmentRequests || !offering.deliveryModes.includes(care.deliveryMode)) throw new ConflictException('Provider service is no longer eligible for the requested delivery mode');
          if (care.deliveryMode !== CareDeliveryMode.IN_PERSON && dto.providerLocationReference) throw new BadRequestException('Provider location is only valid for an in-person Care Appointment');
  ...
  async getProvider(user: User, reference: string) { const provider = await this.operationalProvider(user); const row = await this.readBuilder().where('appointment.reference = :reference AND appointment.providerId = :providerId', { reference, providerId: provider.id }).getOne(); if (!row) this.notFound(); return this.map(row, true); }
  async updateMeetingLink(user: User, reference: string, meetingUrl: string | null) { const provider = await this.operationalProvider(user); return this.appointments.manager.transaction(async (manager) => { const appointment = await manager.getRepository(CareAppointment).findOne({ where: { reference, providerId: provider.id }, lock: { mode: 'pessimistic_write' } }); if (!appointment) this.notFound(); if (appointment.deliveryMode !== CareDeliveryMode.VIRTUAL) throw new ConflictException('Meeting links are only valid for virtual Care Appointments'); if (![CareAppointmentStatus.SCHEDULED, CareAppointmentStatus.CONFIRMED, CareAppointmentStatus.IN_PROGRESS].includes(appointment.status)) throw new ConflictException(`Care Appointment in ${appointment.status} cannot update its meeting link`); if (meetingUrl && !this.isHttpsUrl(meetingUrl)) throw new BadRequestException('meetingUrl must be a valid HTTPS URL'); appointment.meetingUrl = meetingUrl; await manager.getRepository(CareAppointment).save(appointment); return this.getMapped(manager, appointment.id, true); }); }
  ...
  async getMine(user: User, reference: string) { const patient = await this.patient(user.id); const row = await this.readBuilder().where('appointment.reference = :reference AND appointment.patientId = :patientId', { reference, patientId: patient.id }).getOne(); if (!row) this.notFound(); return this.map(row, true); }
  ...
  private async getMapped(manager: EntityManager, id: string, includeMeetingUrl = false) { return this.map(await this.readBuilder(manager).where('appointment.id = :id', { id }).getOneOrFail(), includeMeetingUrl); }
  private map(row: CareAppointment, includeMeetingUrl = false) { return { appointmentReference: row.reference, careRequestReference: row.careRequest.reference, status: row.status, deliveryMode: row.deliveryMode, service: { code: row.providerCareService.definition.code, name: row.providerCareService.definition.name }, provider: { providerReference: row.provider.providerReference, displayName: row.provider.displayName, providerType: row.provider.providerType }, providerLocation: row.providerLocation ? { locationReference: row.providerLocation.locationReference, name: row.providerLocation.name, addressLine1: row.providerLocation.addressLine1, addressLine2: row.providerLocation.addressLine2, city: row.providerLocation.city, stateOrRegion: row.providerLocation.state, postalCode: row.providerLocation.postalCode, countryCode: row.providerLocation.countryCode } : null, scheduledDate: row.scheduledDate, scheduledTimeFrom: row.scheduledTimeFrom, scheduledTimeTo: row.scheduledTimeTo, timezone: row.timezone, ...(includeMeetingUrl && row.deliveryMode === CareDeliveryMode.VIRTUAL ? { meetingUrl: row.meetingUrl } : {}), notes: row.notes, createdAt: row.createdAt, updatedAt: row.updatedAt }; }
  private isHttpsUrl(value: string) { try { const url = new URL(value); return url.protocol === 'https:' && Boolean(url.hostname); } catch { return false; } }
