...
@Entity("clinical_orders")
@Index("UQ_clinical_orders_reference", ["reference"], { unique: true })
@Index("IDX_clinical_orders_patient_status_created", [
  "patientId",
  "status",
  "createdAt",
])
@Index("IDX_clinical_orders_provider_status_created", [
  "orderingProviderId",
  "status",
  "createdAt",
])
...
export class ClinicalOrder {
  ...
  @Column({ name: "ordering_user_id", type: "uuid" }) orderingUserId!: string;
  ...
  @Column({
    ...
    enumName: "clinical_order_type_enum",
  })
  type!: ClinicalOrderType;
  @Column({
    type: "enum",
    enum: ClinicalOrderStatus,
    enumName: "clinical_order_status_enum",
  })
  ...
  updatedAt!: Date;
  @OneToOne(() => ClinicalPrescriptionDetail, (d) => d.clinicalOrder)
  prescription!: ClinicalPrescriptionDetail | null;
}
