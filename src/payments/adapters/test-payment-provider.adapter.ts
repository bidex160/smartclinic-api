...
export class TestPaymentProviderAdapter implements PaymentProviderAdapter {
  ...
  async initializePayment(input: InitializePaymentInput): Promise<InitializePaymentResult> { const providerReference = `TEST-${input.idempotencyKey}`; this.payments.set(providerReference, { amount: input.amount, currency: input.currency, failed: input.idempotencyKey.toUpperCase().startsWith('FAIL') }); return { providerCode: 'TEST', providerReference, status: PaymentAttemptStatus.AWAITING_CUSTOMER_ACTION }; }
