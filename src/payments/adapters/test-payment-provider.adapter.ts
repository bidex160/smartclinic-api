...
export class TestPaymentProviderAdapter implements PaymentProviderAdapter {
  private readonly payments = new Map<
    string,
    { amount: string; currency: string; failed: boolean }
  >();
  async initializePayment(
    input: InitializePaymentInput,
  ): Promise<InitializePaymentResult> {
    const providerReference = input.paymentReference;
    this.payments.set(providerReference, {
      amount: input.amount,
      currency: input.currency,
      failed: input.idempotencyKey.toUpperCase().startsWith("FAIL"),
    });
    return {
      providerCode: "TEST",
      providerReference,
      status: PaymentAttemptStatus.AWAITING_CUSTOMER_ACTION,
      checkoutUrl: `https://checkout.test/${providerReference}`,
      accessCode: null,
    };
  }
  ...
  parseWebhook(): { type: string; reference: string | null } {
    return { type: "unsupported", reference: null };
  }
}
