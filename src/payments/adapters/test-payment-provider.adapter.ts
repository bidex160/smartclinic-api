...
export class TestPaymentProviderAdapter implements PaymentProviderAdapter {
  ...
  async verifyPayment(providerReference: string): Promise<VerifyPaymentResult> { const payment = this.payments.get(providerReference); if (!payment) return { succeeded: false, status: PaymentAttemptStatus.FAILED, providerReference, amount: '0.00', currency: 'NGN', occurredAt: new Date() }; return { succeeded: !payment.failed, status: payment.failed ? PaymentAttemptStatus.FAILED : PaymentAttemptStatus.SUCCEEDED, providerReference, amount: payment.amount, currency: payment.currency, occurredAt: new Date() }; }
