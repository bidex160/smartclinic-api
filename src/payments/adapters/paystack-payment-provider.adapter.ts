...
export function decimalToSubunit(amount: string): string {
  if (!/^\d+(?:\.\d{1,2})?$/.test(amount))
    throw new BadRequestException(
      "Payment amount is not a valid two-decimal monetary value",
    );
  const [whole, fraction = ""] = amount.split(".");
  return `${BigInt(whole) * 100n + BigInt(fraction.padEnd(2, "0"))}`;
}
export function subunitToDecimal(amount: number | string): string {
...
export class PaystackPaymentProviderAdapter implements PaymentProviderAdapter {
  ...
  async initializePayment(
    ...
    const body: Record<string, unknown> = {
      ...
      amount: decimalToSubunit(input.amount),
    ...
    if (
      ...
      !envelope.data.authorization_url ||  !envelope.data.access_code
    )
  ...
  async verifyPayment(reference: string): Promise<VerifyPaymentResult> {
    const envelope = await this.request<PaystackVerifyData>(
    ...
    return {
      ...
      amount: subunitToDecimal(data.amount),
  ...
  parseWebhook(rawBody: Buffer): PaymentProviderWebhookEvent {
    try {
      ...
      return {
        type: typeof event.event === "string" ? event.event : "unsupported",
        reference:
      ...
      throw new BadRequestException("Malformed payment webhook");
    }
  }
  private secret(): string {
    const secret = this.config.payments.paystack.secretKey;
    if (!secret)
