...
import { PatientProviderConnectionFundingPurpose, PatientProviderConnectionFundingStatus } from '../patient-provider-connections/enums/patient-provider-connection-funding.enum';
...
export class PaymentFlowService {
  ...
  async applyProviderVerification(
    ...
    if (attempt.patientProviderConnectionFundingId) return this.applyPatientProviderConnectionVerification(attempt.id, null, verified) as never;
    return this.applyVerification(attempt.id, null, verified);
  }

  async getPatientProviderConnectionFunding(reference: string, userId: string) {
    const connection = await this.bookings.manager.getRepository(PatientProviderConnection).findOne({ where: { reference, initiatedByUserId: userId } }); if (!connection) throw new NotFoundException('Patient connection was not found');
    const fundings = await this.bookings.manager.getRepository(PatientProviderConnectionFunding).find({ where: { connectionId: connection.id }, order: { createdAt: 'ASC' } }); const fundingIds=fundings.map(f=>f.id); const attempts=fundingIds.length?await this.attempts.find({where:{patientProviderConnectionFundingId:In(fundingIds)},order:{createdAt:'DESC'}}):[]; const latest=new Map<string,PaymentAttempt>(); for(const attempt of attempts)if(attempt.patientProviderConnectionFundingId&&!latest.has(attempt.patientProviderConnectionFundingId))latest.set(attempt.patientProviderConnectionFundingId,attempt); return this.patientConnectionFundingResponse(connection,fundings,latest);
  }

  async initializePatientProviderConnectionFunding(reference:string,userId:string){
    if(!this.commissions)throw new ConflictException('Provider commission is not available'); const prepared=await this.bookings.manager.transaction(async manager=>{const connection=await manager.getRepository(PatientProviderConnection).findOne({where:{reference,initiatedByUserId:userId},relations:{initiatedBy:true},lock:{mode:'pessimistic_write'}});if(!connection)throw new NotFoundException('Patient connection was not found');if(connection.status!==PatientProviderConnectionStatus.AWAITING_FUNDING)throw new ConflictException('Patient connection is not awaiting funding');const funding=await manager.getRepository(PatientProviderConnectionFunding).findOne({where:{connectionId:connection.id,status:PatientProviderConnectionFundingStatus.PENDING},order:{createdAt:'DESC'},lock:{mode:'pessimistic_write'}});if(!funding)throw new ConflictException('No payable Patient connection funding obligation exists');await this.commissions!.requireForProvider(connection.providerId,manager);return{connection,funding};});
    const active=await this.attempts.findOne({where:{patientProviderConnectionFundingId:prepared.funding.id,status:In([PaymentAttemptStatus.CREATED,PaymentAttemptStatus.AWAITING_CUSTOMER_ACTION,PaymentAttemptStatus.PENDING_CONFIRMATION])},order:{createdAt:'DESC'}});if(active)return this.patientConnectionAttemptResponse(prepared.connection,prepared.funding,active);const email=prepared.connection.initiatedBy?.email;if(!email||!isEmail(email))throw new BadRequestException('A valid account email is required to initialize payment');const idempotencyKey=`PATIENT-CONNECTION-${randomBytes(16).toString('hex')}`;const paymentReference=`SC-PAY-${randomBytes(12).toString('hex')}`;const initialized=await this.provider.initializePayment({amount:this.fromMinor(BigInt(prepared.funding.amountMinor)),currency:prepared.funding.currency,idempotencyKey,bookingReference:reference,customerEmail:email,paymentReference,callbackUrl:this.callbackUrl(reference,true)});
    return this.bookings.manager.transaction(async manager=>{const funding=await manager.getRepository(PatientProviderConnectionFunding).findOne({where:{id:prepared.funding.id},lock:{mode:'pessimistic_write'}});if(!funding||funding.status!==PatientProviderConnectionFundingStatus.PENDING)throw new ConflictException('Patient connection funding is no longer payable');const repo=manager.getRepository(PaymentAttempt);const raced=await repo.findOne({where:{patientProviderConnectionFundingId:funding.id,status:In([PaymentAttemptStatus.CREATED,PaymentAttemptStatus.AWAITING_CUSTOMER_ACTION,PaymentAttemptStatus.PENDING_CONFIRMATION])}});if(raced)return this.patientConnectionAttemptResponse(prepared.connection,funding,raced);const attempt=await repo.save(repo.create({bookingFundingId:null,fastTrackRequestId:null,careRequestFundingId:null,patientProviderConnectionFundingId:funding.id,amount:this.fromMinor(BigInt(funding.amountMinor)),currency:funding.currency,status:initialized.status,idempotencyKey,providerCode:initialized.providerCode,providerReference:initialized.providerReference,checkoutUrl:initialized.checkoutUrl,accessCode:initialized.accessCode}));return this.patientConnectionAttemptResponse(prepared.connection,funding,attempt);});
  }
  async verifyLatestPatientProviderConnectionFunding(reference:string,userId:string){const connection=await this.bookings.manager.getRepository(PatientProviderConnection).findOne({where:{reference,initiatedByUserId:userId}});if(!connection)throw new NotFoundException('Patient connection was not found');const funding=await this.bookings.manager.getRepository(PatientProviderConnectionFunding).findOne({where:{connectionId:connection.id,status:PatientProviderConnectionFundingStatus.PENDING},order:{createdAt:'DESC'}});if(!funding)return this.getPatientProviderConnectionFunding(reference,userId);const attempt=await this.attempts.findOne({where:{patientProviderConnectionFundingId:funding.id},order:{createdAt:'DESC'}});if(!attempt?.providerReference)throw new ConflictException('No Patient connection payment attempt is available to verify');if(attempt.status!==PaymentAttemptStatus.SUCCEEDED){await this.claimVerification(attempt.id);const verified=await this.provider.verifyPayment(attempt.providerReference);await this.applyPatientProviderConnectionVerification(attempt.id,userId,verified);}return this.getPatientProviderConnectionFunding(reference,userId);}
  private async applyPatientProviderConnectionVerification(attemptId:string,actorUserId:string|null,verified:VerifyPaymentResult){if(!this.earnings)throw new ConflictException('Provider earnings accounting is not available');return this.bookings.manager.transaction(async manager=>{const attemptRepo=manager.getRepository(PaymentAttempt);const attempt=await attemptRepo.findOne({where:{id:attemptId},lock:{mode:'pessimistic_write'}});if(!attempt?.patientProviderConnectionFundingId)throw new NotFoundException('Patient connection payment attempt was not found');const fundingRepo=manager.getRepository(PatientProviderConnectionFunding);const funding=await fundingRepo.findOne({where:{id:attempt.patientProviderConnectionFundingId},lock:{mode:'pessimistic_write'}});if(!funding)throw new NotFoundException('Patient connection funding was not found');const connectionRepo=manager.getRepository(PatientProviderConnection);const connection=await connectionRepo.findOne({where:{id:funding.connectionId},lock:{mode:'pessimistic_write'}});if(!connection)throw new NotFoundException('Patient connection was not found');if(attempt.status===PaymentAttemptStatus.SUCCEEDED&&funding.status===PatientProviderConnectionFundingStatus.PAID)return this.patientConnectionAttemptResponse(connection,funding,attempt);const expected=this.fromMinor(BigInt(funding.amountMinor));if(verified.providerReference!==attempt.providerReference||verified.amount!==attempt.amount||verified.currency!==attempt.currency||attempt.amount!==expected||attempt.currency!==funding.currency){attempt.status=PaymentAttemptStatus.FAILED;await attemptRepo.save(attempt);return this.patientConnectionAttemptResponse(connection,funding,attempt);}if(!verified.succeeded||verified.status!==PaymentAttemptStatus.SUCCEEDED){attempt.status=verified.status;await attemptRepo.save(attempt);return this.patientConnectionAttemptResponse(connection,funding,attempt);}const txRepo=manager.getRepository(PaymentTransaction);let transaction=await txRepo.findOne({where:{providerReference:verified.providerReference}});if(transaction&&transaction.paymentAttemptId!==attempt.id)throw new ConflictException('Verified provider transaction belongs to another payment attempt');if(!transaction)transaction=await txRepo.save(txRepo.create({paymentAttemptId:attempt.id,parentTransactionId:null,transactionType:PaymentTransactionType.COLLECTION,status:PaymentTransactionStatus.SUCCEEDED,amount:verified.amount,currency:verified.currency,providerReference:verified.providerReference,occurredAt:verified.occurredAt}));const sourceType=funding.purpose===PatientProviderConnectionFundingPurpose.CONVERSION_ADDITIONAL?ProviderEarningSourceType.PATIENT_REGISTRATION:connection.originalIntent===PatientProviderConnectionType.NEW_PATIENT_REGISTRATION?ProviderEarningSourceType.PATIENT_REGISTRATION:ProviderEarningSourceType.PATIENT_LINKING;await this.earnings!.createHeldPatientConnectionEarning(manager,{providerId:connection.providerId,sourceType,sourceReference:connection.reference,grossAmountMinor:funding.amountMinor,currency:funding.currency,paymentTransaction:transaction});attempt.status=PaymentAttemptStatus.SUCCEEDED;await attemptRepo.save(attempt);funding.status=PatientProviderConnectionFundingStatus.PAID;funding.paidAt=verified.occurredAt;await fundingRepo.save(funding);const remaining=await fundingRepo.count({where:{connectionId:connection.id,status:PatientProviderConnectionFundingStatus.PENDING}});if(remaining===0&&connection.status===PatientProviderConnectionStatus.AWAITING_FUNDING){const from=connection.status;connection.status=PatientProviderConnectionStatus.SUBMITTED;await connectionRepo.save(connection);await manager.getRepository(PatientProviderConnectionHistory).save({connectionId:connection.id,fromStatus:from,toStatus:connection.status,actorUserId,reasonCode:'FUNDING_SATISFIED_SUBMITTED',reasonNote:null});}return this.patientConnectionAttemptResponse(connection,funding,attempt);});}
  private patientConnectionAttemptResponse(connection:PatientProviderConnection,funding:PatientProviderConnectionFunding,attempt:PaymentAttempt|null){return{connectionReference:connection.reference,purpose:funding.purpose,amountMinor:Number(funding.amountMinor),currency:funding.currency,fundingStatus:funding.status,paid:funding.status===PatientProviderConnectionFundingStatus.PAID||funding.status===PatientProviderConnectionFundingStatus.SATISFIED_FREE,paymentAttemptStatus:attempt?.status??null,paymentReference:attempt?.providerReference??null,checkoutUrl:attempt?.checkoutUrl??null,accessCode:attempt?.accessCode??null,paidAt:funding.paidAt};}
  private patientConnectionFundingResponse(connection:PatientProviderConnection,fundings:PatientProviderConnectionFunding[],attempts:Map<string,PaymentAttempt>){return{connectionReference:connection.reference,status:connection.status,fundings:fundings.map(f=>this.patientConnectionAttemptResponse(connection,f,attempts.get(f.id)??null)),fundingSatisfied:fundings.length>0&&fundings.every(f=>f.status===PatientProviderConnectionFundingStatus.PAID||f.status===PatientProviderConnectionFundingStatus.SATISFIED_FREE)};}
