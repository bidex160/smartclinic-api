...
export class AuthController {
  ...
  @ApiOperation({ summary: "Request password reset instructions by email" })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: "Authenticate with email or phone number and password",
  })
  ...
  async login(
    ...
    return this.withCookie(await this.auth.login(dto), response);
  }
  @Post("refresh")
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
  ...
  async refresh(
    ...
    return this.withCookie(
      await this.auth.refresh(this.cookie(request.headers.cookie)),
      response,
    );
  }
  @Post("logout")
  ...
  async logout(
    ...
    await this.auth.logout(this.cookie(request.headers.cookie));
    this.clear(response);
  }
  @Post("logout-all")
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Revoke every refresh session for the authenticated user",
  })
  async logoutAll(
    @Req() request: { user: User },
    @Res({ passthrough: true }) response: Response,
  ): Promise<void> {
    await this.auth.logoutAll(request.user.id);
    this.clear(response);
  }
  @Get("me")
  @UseGuards(JwtAuthGuard)
  ...
  me(@Req() request: { user: User }): UserResponseDto {
  ...
  private withCookie(
    ...
    response.cookie("refreshToken", token, {
      ...
      sameSite: c.auth.cookieSameSite,
      domain: c.auth.cookieDomain,
      path: "/api/v1/auth",
      maxAge: c.auth.refreshTokenTtl * 1000,
    });
    return result;
  }
  ...
  private cookie(header?: string): string | undefined {
    return header
      ?.split(";")
