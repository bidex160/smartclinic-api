...
describe('ProviderOnboardingService', () => {
  ...
  let providerRows: any[], userRows: any[], credentialRows: any[], providers: any, users: any, credentials: any, context: any, readiness: any, referrals: any, notifications: any, subject: ProviderOnboardingService;
  beforeEach(() => {
    ...
    readiness = { evaluate: jest.fn(async (providerId) => ({ profileComplete: true, hasActiveCapability: true, providerLocationReady: true, hasAvailability: true, blockers: [], capabilityCount: 1, activeCapabilityCount: 1, locationCount: 1, activeLocationCount: 1, availabilityCount: 1 })), evaluateAccountReadiness: jest.fn(async () => ({ profileComplete: true, blockers: [] })) };
    referrals = { ensureReferralCode: jest.fn(), captureProvider: jest.fn() };
    notifications = { createTransactionalNotification: jest.fn() };
    subject = new ProviderOnboardingService(providers, users, credentials, context, readiness, referrals, notifications);
  });
  ...
  it('registers HOSPITAL through the existing pending onboarding lifecycle', async () => { const result = await subject.register({ ...dto, displayName: 'University Hospital', email: 'hospital@example.test', providerType: ProviderType.HOSPITAL }); expect(result).toMatchObject({ providerType: ProviderType.HOSPITAL, status: ProviderStatus.PENDING, onboardingStatus: ProviderOnboardingStatus.DRAFT }); expect(providerRows[0].providerType).toBe(ProviderType.HOSPITAL); });
  ...
  it('submits a complete provider without Health Check configuration', async () => { await subject.register(dto); const result = await subject.submit(userRows[0]); expect(result).toMatchObject({ onboardingStatus: ProviderOnboardingStatus.SUBMITTED, status: ProviderStatus.PENDING }); expect(readiness.evaluateAccountReadiness).toHaveBeenCalledWith(providerRows[0].id, expect.anything()); expect(notifications.createTransactionalNotification).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ type: 'PROVIDER_ONBOARDING_SUBMITTED', email: { enabled: true } })); });
  it('rejects submission with account readiness blockers and a missing provider link', async () => { await subject.register(dto); readiness.evaluateAccountReadiness.mockResolvedValueOnce({ profileComplete: false, blockers: [ProviderOnboardingBlocker.PROFILE_INCOMPLETE] }); await expect(subject.submit(userRows[0])).rejects.toBeInstanceOf(ConflictException); providerRows = []; await expect(subject.get(userRows[0])).rejects.toBeInstanceOf(ForbiddenException); });
