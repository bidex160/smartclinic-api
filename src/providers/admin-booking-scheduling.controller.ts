...
@ApiTags("Admin booking scheduling")
@ApiBearerAuth()
...
export class AdminBookingSchedulingController {
  ...
  @Post(":reference/schedule")
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary:
      "Confirm a provider-assigned booking appointment (ADMIN or OPERATIONS)",
  })
  @ApiOkResponse({ type: AdminBookingScheduleResponseDto })
  schedule(
    @Param() { reference }: BookingReferenceParamsDto,
    @Req() request: { user: User },
    @Body() dto: ScheduleBookingDto,
  ) {
    return this.scheduling.schedule(reference, request.user.id, dto);
  }
}
