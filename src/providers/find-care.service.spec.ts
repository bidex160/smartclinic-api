...
const provider = () => ({
  ...
  careServices: [{ id: 'internal-service', isActive: true, priceMinor: '250000', currency: 'NGN', descriptionOverride: null, supportsAppointmentRequests: true, definition: { id: 'internal-definition', code: 'GENERAL_CONSULTATION', name: 'General consultation', description: 'Consult a clinician', isActive: true } }],
});
...
describe('FindCareService', () => {
  ...
  it('applies service, provider type, and authoritative location filters', async () => {
    ...
    await service.providersList({ serviceCode: 'GENERAL_CONSULTATION', providerType: 'CLINIC' as any, countryCode: 'NG', stateOrRegion: 'Lagos', city: 'Ikeja', page: 1, limit: 20 });
    const sql = builder.andWhere.mock.calls.map((call: any[]) => call[0]).filter((v: unknown) => typeof v === 'string').join(' ');
    expect(sql).toContain('definition.code = :serviceCode'); expect(sql).toContain('provider.providerType = :providerType'); expect(sql).toContain('location.countryCode'); expect(sql).toContain('location.state'); expect(sql).toContain('location.city');
  });
  ...
  it('returns only safe public fields and integer minor-unit prices', async () => {
    ...
    expect(result).toMatchObject({ providerReference: 'SCPR-ABCDEF0123456789', services: [{ code: 'GENERAL_CONSULTATION', priceMinor: 250000, currency: 'NGN' }] });
