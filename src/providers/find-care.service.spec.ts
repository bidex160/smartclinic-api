...
const provider = () => ({
  ...
  careServices: [{ id: 'internal-service', isActive: true, descriptionOverride: null, supportsAppointmentRequests: true, supportsFastTrack: true, fastTrackFeeMinor: '500000', fastTrackCurrency: 'NGN', deliveryOptions: [{ deliveryMode: CareDeliveryMode.IN_PERSON, priceMinor: '250000', currency: 'NGN' }, { deliveryMode: CareDeliveryMode.VIRTUAL, priceMinor: '180000', currency: 'NGN' }], definition: { id: 'internal-definition', code: 'GENERAL_CONSULTATION', name: 'General consultation', description: 'Consult a clinician', isActive: true } }],
});
...
describe('FindCareService', () => {
  ...
  it('discovers a virtual-only provider without Health Check services or locations', async () => { const virtualOnly: any = provider(); virtualOnly.locations = []; virtualOnly.healthCheckServices = []; const builder = qb([virtualOnly]); const service = new FindCareService({ createQueryBuilder: jest.fn().mockReturnValue(builder) } as any, {} as any); const result = await service.providersList({ serviceCode: 'GENERAL_CONSULTATION', deliveryMode: CareDeliveryMode.VIRTUAL, page: 1, limit: 20 }); expect(result.items).toHaveLength(1); expect(result.items[0].services[0].deliveryOptions).toEqual(expect.arrayContaining([expect.objectContaining({ deliveryMode: CareDeliveryMode.VIRTUAL })])); });
  ...
  it('supports provider-name-first q-only discovery without geography', async () => { const builder = qb(); const service = new FindCareService({ createQueryBuilder: jest.fn().mockReturnValue(builder) } as any, {} as any); await expect(service.providersList({ q: 'Primed', page: 1, limit: 20 })).resolves.toMatchObject({ total: 1 }); const brackets = builder.andWhere.mock.calls.find(([value]: any[]) => typeof value !== 'string')?.[0]; const where = { where: jest.fn().mockReturnThis(), orWhere: jest.fn().mockReturnThis() }; brackets.whereFactory(where); expect(where.where).toHaveBeenCalledWith('provider.displayName ILIKE :search', { search: '%Primed%' }); expect(where.orWhere).toHaveBeenCalledWith('definition.name ILIKE :search', { search: '%Primed%' }); });
  it.each([{ countryCode: 'NG' }, { stateOrRegion: 'Lagos' }, { city: 'Ikeja' }])('applies optional name-search geography refinement %#', async (place) => { const builder = qb(); const service = new FindCareService({ createQueryBuilder: jest.fn().mockReturnValue(builder) } as any, {} as any); await service.providersList({ q: 'Primed', ...place, page: 1, limit: 20 }); const sql = builder.andWhere.mock.calls.map(([value]: any[]) => typeof value === 'string' ? value : '').join(' '); expect(sql).toMatch(/provider\.(countryCode|stateOrRegion|city)/); });
  it('applies explicit FastTrack-only capability filters and leaves ordinary discovery unchanged', async () => { const fast = qb(); const service = new FindCareService({ createQueryBuilder: jest.fn().mockReturnValue(fast) } as any, {} as any); await service.providersList({ q: 'Primed', fastTrackOnly: true, page: 1, limit: 20 }); expect(fast.andWhere).toHaveBeenCalledWith('careService.supportsFastTrack = true'); expect(fast.andWhere).toHaveBeenCalledWith('careService.fastTrackFeeMinor IS NOT NULL'); expect(fast.andWhere).toHaveBeenCalledWith('careService.fastTrackCurrency IS NOT NULL'); const ordinary = qb(); await new FindCareService({ createQueryBuilder: jest.fn().mockReturnValue(ordinary) } as any, {} as any).providersList({ q: 'Primed', page: 1, limit: 20 }); expect(ordinary.andWhere).not.toHaveBeenCalledWith('careService.supportsFastTrack = true'); });
