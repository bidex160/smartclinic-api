...
describe('ProviderOffersService', () => {
  ...
  beforeEach(() => {
    ...
    matching = { acceptOffer: jest.fn(async () => { own.status = ProviderAssignmentStatus.ACCEPTED; own.respondedAt = new Date(); own.acceptedAt = new Date(); }), declineOffer: jest.fn(async (_id, _provider, reason) => { own.status = ProviderAssignmentStatus.DECLINED; own.respondedAt = new Date(); own.reasonNote = reason; }) };
  ...
  it('accepts an owned offer using resolved provider identity', async () => { const result = await subject.accept(user, assignmentId); expect(matching.acceptOffer).toHaveBeenCalledWith(assignmentId, providerId); expect(result.status).toBe(ProviderAssignmentStatus.ACCEPTED); });
