...
describe('AdminProvidersService', () => {
  ...
  beforeEach(() => {
    ...
    const referrals = { qualifyProvider: jest.fn().mockResolvedValue(undefined), logQualificationFailure: jest.fn() };
    subject = new AdminProvidersService(providers, users, assignments, reservations, capabilities, locations, readiness, referrals as never);
  });
  ...
  it('approves submitted onboarding, qualifies its referral in the same transaction, and activates', async () => { provider.userId = user.id; user.roles = [UserRole.PROVIDER]; expect(await subject.approve(provider.id, creatorId)).toMatchObject({ status: ProviderStatus.ACTIVE, onboardingStatus: ProviderOnboardingStatus.APPROVED }); expect((subject as any).referrals.qualifyProvider).toHaveBeenCalledWith(provider.id, expect.anything()); expect((await subject.suspend(provider.id)).status).toBe(ProviderStatus.SUSPENDED); expect((await subject.activate(provider.id)).status).toBe(ProviderStatus.ACTIVE); });
