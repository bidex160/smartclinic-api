...
describe('AdminProvidersService', () => {
  ...
  beforeEach(() => {
    ...
    provider = { id: '10000000-0000-4000-8000-000000000001', displayName: 'SmartClinic Ikeja', professionalReference: null, status: ProviderStatus.PENDING, userId: null, user: null, createdAt: new Date(), updatedAt: new Date(), deletedAt: null };
  ...
  it('creates an unlinked provider in safe PENDING status', async () => { const result = await subject.create({ displayName: 'New Provider' }); expect(result).toMatchObject({ displayName: 'New Provider', status: ProviderStatus.PENDING, linkedUser: null, capabilityCount: 0, locationCount: 0 }); });
  ...
  it('activates and suspends operational status explicitly', async () => { expect((await subject.activate(provider.id)).status).toBe(ProviderStatus.ACTIVE); expect((await subject.suspend(provider.id)).status).toBe(ProviderStatus.SUSPENDED); });
