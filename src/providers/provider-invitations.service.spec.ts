...
describe('ProviderInvitationsService', () => {
  let provider: any, creator: any, invitationRows: any[], userRows: any[], credentialRows: any[], invitations: any, providers: any, users: any, credentials: any, subject: ProviderInvitationsService;
  beforeEach(() => {
    ...
    subject = new ProviderInvitationsService(invitations, providers, users, { providerInvitations: { ttlSeconds: 3600 } } as never);
  });

  async function created() { return subject.create(provider.id, 'Provider@Example.COM', creator.id); }

  it('creates an invitation, returns the raw token once, and stores only its hash', async () => { const result = await created(); expect(result.invitationToken).toMatch(/^[A-Za-z0-9_-]{43}$/); expect(invitationRows[0].tokenHash).toBe(createHash('sha256').update(result.invitationToken).digest('hex')); expect(JSON.stringify(invitationRows[0])).not.toContain(result.invitationToken); expect(invitationRows[0].emailNormalized).toBe('provider@example.com'); });
  ...
  it('lists summaries without raw tokens or hashes', async () => { const creation = await created(); const result: any[] = await subject.list(provider.id); expect(result[0]).not.toHaveProperty('invitationToken'); expect(result[0]).not.toHaveProperty('tokenHash'); expect(JSON.stringify(result)).not.toContain(creation.invitationToken); });
  it('revokes a pending invitation and makes its token unusable', async () => { const creation = await created(); const revoked = await subject.revoke(creation.id); expect(revoked.status).toBe(ProviderInvitationStatus.REVOKED); await expect(subject.inspect(creation.invitationToken)).rejects.toBeInstanceOf(NotFoundException); });
  it('inspects a valid token with masked email and rejects invalid/expired tokens', async () => { const creation = await created(); await expect(subject.inspect(creation.invitationToken)).resolves.toMatchObject({ providerDisplayName: provider.displayName, invitedEmail: 'p******@example.com' }); await expect(subject.inspect('x'.repeat(43))).rejects.toBeInstanceOf(NotFoundException); invitationRows[0].expiresAt = new Date(Date.now() - 1); await expect(subject.inspect(creation.invitationToken)).rejects.toMatchObject({ status: 410 }); });

  it('accepts once, creates a securely hashed provider user, links it, and preserves provider data', async () => { const creation = await created(); const result = await subject.accept(creation.invitationToken, { displayName: 'Ada Provider', password: 'very-secure-password' }); expect(result).toEqual({ providerDisplayName: provider.displayName, email: 'provider@example.com', status: ProviderInvitationStatus.ACCEPTED, loginRequired: true }); const account = userRows.find((row) => row.emailNormalized === 'provider@example.com'); expect(account.roles).toEqual([UserRole.PROVIDER]); expect(account.status).toBe(UserStatus.ACTIVE); expect(provider.userId).toBe(account.id); expect(invitationRows[0]).toMatchObject({ status: ProviderInvitationStatus.ACCEPTED, acceptedAt: expect.any(Date) }); expect(await bcrypt.compare('very-secure-password', credentialRows[0].passwordHash)).toBe(true); expect(provider.services).toEqual([{ id: 'capability-preserved' }]); expect(result).not.toHaveProperty('accessToken'); });
  it('rejects token replay and uses pessimistic locks to prevent double acceptance', async () => { const creation = await created(); await subject.accept(creation.invitationToken, { displayName: 'Ada', password: 'very-secure-password' }); await expect(subject.accept(creation.invitationToken, { displayName: 'Ada', password: 'very-secure-password' })).rejects.toBeInstanceOf(NotFoundException); expect(invitations.findOne).toHaveBeenCalledWith(expect.objectContaining({ lock: { mode: 'pessimistic_write' } })); });
  it('rejects the existing-account branch without overwriting credentials', async () => { const creation = await created(); userRows.push({ id: 'existing', emailNormalized: 'provider@example.com', roles: [UserRole.USER], status: UserStatus.ACTIVE }); await expect(subject.accept(creation.invitationToken, { displayName: 'Takeover', password: 'very-secure-password' })).rejects.toBeInstanceOf(ConflictException); expect(credentials.save).not.toHaveBeenCalled(); expect(provider.userId).toBeNull(); });
});
