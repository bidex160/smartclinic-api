...
describe('ProviderInvitationsService', () => {
  ...
  it('sends text and HTML with stable token-free idempotency and omits token material after success', async () => { emailProvider.sendTransactionalEmail.mockResolvedValue({ outcome: EmailSendOutcome.SENT }); const result = await created(); expect(result).toMatchObject({ deliveryStatus: 'SENT' }); expect(result).not.toHaveProperty('manualInvitationLink'); const message = emailProvider.sendTransactionalEmail.mock.calls[0][0]; expect(message).toEqual(expect.objectContaining({ to: 'provider@example.com', text: expect.stringContaining('used only once'), html: expect.stringContaining('<a href='), idempotencyKey: 'provider-invitation:invitation-1:initial' })); const rawToken = message.html.match(/setup\/([A-Za-z0-9_-]{43})/)?.[1]; expect(message.idempotencyKey).not.toContain(rawToken); expect(JSON.stringify(result)).not.toContain('resend-message-id'); });
