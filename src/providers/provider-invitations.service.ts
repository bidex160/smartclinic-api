...
import {
  AcceptProviderInvitationDto,
  AcceptedProviderInvitationResponseDto,
  AdminProviderInvitationSummaryDto,
  CreatedProviderInvitationResponseDto,
  ProviderInvitationDeliveryStatus,
  PublicProviderInvitationResponseDto,
} from "./dto/provider-invitation.dto";
...
export class ProviderInvitationsService {
  ...
  constructor(
    @InjectRepository(ProviderInvitation)
    private readonly invitations: Repository<ProviderInvitation>,
  ...
  async createProvider(
    ...
    creatorId: string,
  ): Promise<AdminCreatedProviderResponseDto> {
    const email = dto.email.trim().toLowerCase();
    const rawToken = randomBytes(32).toString("base64url");
    const now = new Date();
    let result: { provider: Provider; invitation: ProviderInvitation };
    try {
      result = await this.invitations.manager.transaction(async (manager) => {
        const providerRepository = manager.getRepository(Provider);
        const invitationRepository = manager.getRepository(ProviderInvitation);
        ...
        if (
          ...
          throw new ConflictException(
            "An account already exists for this email; use existing-user linking",
          );
        if (
          await providerRepository.exists({
            where: { email },
            withDeleted: true,
          })
        )
          throw new ConflictException(
            "A provider already exists for this email",
          );
        const provider = await providerRepository.save(
          providerRepository.create({
            ...
            onboardingStatus: ProviderOnboardingStatus.INVITED,
            submittedAt: null,
            reviewedAt: null,
        ...
        const invitation = await invitationRepository.save(
          invitationRepository.create({
            ...
            status: ProviderInvitationStatus.PENDING,
            expiresAt: new Date(
              now.getTime() + this.config.providerInvitations.ttlSeconds * 1000,
            ),
            acceptedAt: null,
            revokedAt: null,
      ...
      if (error instanceof QueryFailedError)
        throw new ConflictException(
          "A provider account or invitation already exists for this email",
        );
      throw error;
    }
  ...
  async create(
    ...
    email: string,
    creatorId: string,
  ): Promise<CreatedProviderInvitationResponseDto> {
    ...
    const invitation = await this.invitations.manager.transaction(
      async (manager) => {
        ...
        if (existing) {
          existing.status = ProviderInvitationStatus.EXPIRED;
    ...
    if (hydrated.provider.onboardingStatus === ProviderOnboardingStatus.DRAFT) {
      hydrated.provider.onboardingStatus = ProviderOnboardingStatus.INVITED;
      await this.providers.save(hydrated.provider);
    }
    return this.deliver(hydrated, rawToken);
  }

  async list(providerId: string): Promise<AdminProviderInvitationSummaryDto[]> {
    await this.requireProvider(providerId);
    const rows = await this.invitations.find({
      where: { providerId },
      relations: { provider: true, createdBy: true },
      order: { createdAt: "DESC", id: "DESC" },
    });
    return rows.map((row) => this.adminSummary(row));
  }

  async revoke(id: string): Promise<AdminProviderInvitationSummaryDto> {
    await this.invitations.manager.transaction(async (manager) => {
      const repository = manager.getRepository(ProviderInvitation);
      const invitation = await repository.findOne({
        where: { id },
        lock: { mode: "pessimistic_write" },
      });
      if (!invitation)
        throw new NotFoundException("Provider invitation not found");
      ...
      if (invitation.expiresAt <= new Date()) {
        invitation.status = ProviderInvitationStatus.EXPIRED;
        await repository.save(invitation);
        throw new ConflictException("Invitation has expired");
      }
      invitation.status = ProviderInvitationStatus.REVOKED;
  ...
  async inspect(token: string): Promise<PublicProviderInvitationResponseDto> {
    const invitation = await this.findUsable(token);
    return {
      providerDisplayName: invitation.provider.displayName,
      invitedEmail: this.mask(invitation.email),
  ...
  async accept(
    token: string,
    dto: AcceptProviderInvitationDto,
  ): Promise<AcceptedProviderInvitationResponseDto> {
    await this.findUsable(token);
    ...
    try {
      const result = await this.invitations.manager.transaction(
        async (manager) => {
          const invitationRepository =
            manager.getRepository(ProviderInvitation);
          const providerRepository = manager.getRepository(Provider);
          const userRepository = manager.getRepository(User);
          ...
          if (!provider || provider.deletedAt || provider.userId)
            this.invalid();
          if (
            await userRepository.exists({
            ...
            throw new ConflictException(
              "An account already exists for this email; sign in and ask operations to link it",
            );
          ...
          await credentialRepository.save(
            credentialRepository.create({ userId: user.id, passwordHash }),
          );
          ...
          invitation.status = ProviderInvitationStatus.ACCEPTED;
          invitation.acceptedAt = now;
          await invitationRepository.save(invitation);
          return {
            providerDisplayName: provider.displayName,
            email: invitation.email,
            status: ProviderInvitationStatus.ACCEPTED as const,
      ...
      if (error instanceof QueryFailedError)
        throw new ConflictException(
          "Invitation acceptance conflicted with an existing account or provider link",
        );
      throw error;
    }
  }

  private async findUsable(token: string): Promise<ProviderInvitation> {
    const invitation = await this.invitations
      .createQueryBuilder("invitation")
      .withDeleted()
      .leftJoinAndSelect("invitation.provider", "provider")
    ...
    if (
      ...
      !invitation.provider ||
      invitation.provider.deletedAt ||
      invitation.provider.userId
    )
      this.invalid();
    if (invitation.expiresAt <= new Date()) this.expired();
    return invitation;
  }
  ...
  private async requireAdminInvitation(
    id: string,
  ...
  private adminSummary(
    ...
    return {
      id: invitation.id,
      provider: { displayName: invitation.provider.displayName },
      email: invitation.email,
      ...
      createdBy: invitation.createdBy
        ? {
            ...
            displayName: invitation.createdBy.displayName,
          }
        : null,
    };
  }
  private async deliver(
    ...
    try {
      const delivery = await this.emailProvider.sendTransactionalEmail(
        this.invitationEmail(invitation, invitationLink),
      );
      if (delivery.outcome === EmailSendOutcome.SENT)
        return {
          ...this.adminSummary(invitation),
          deliveryStatus: ProviderInvitationDeliveryStatus.SENT,
        };
      return {
        ...this.adminSummary(invitation),
        deliveryStatus: ProviderInvitationDeliveryStatus.MANUAL_REQUIRED,
        manualInvitationLink: invitationLink,
      };
    } catch {
      this.logger.warn(
        `Provider invitation email delivery failed for invitation ${invitation.id}`,
      );
      return {
        ...this.adminSummary(invitation),
        deliveryStatus: ProviderInvitationDeliveryStatus.FAILED,
        manualInvitationLink: invitationLink,
      };
    }
  }
  private createdProvider(provider: Provider) {
    return {
      id: provider.id,
      ...
      readiness: {
        ...
        blockers: [
          ProviderOnboardingBlocker.NO_ACTIVE_CAPABILITY,
  ...
  private invitationLink(token: string): string {
    return `${this.config.providerInvitations.frontendUrl.replace(/\/+$/, "")}/${encodeURIComponent(token)}`;
  }
  ...
  private invalid(): never {
    throw new NotFoundException(
      "Provider invitation is invalid or unavailable",
    );
  }
  private expired(): never {
    throw new HttpException(
      "Provider invitation is invalid or expired",
      HttpStatus.GONE,
    );
  }
}
