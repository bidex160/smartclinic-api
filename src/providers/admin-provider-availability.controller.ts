...
@ApiTags("Admin provider availability")
@ApiBearerAuth()
...
export class AdminProviderAvailabilityController {
  ...
  @ApiOperation({
    summary: "List weekly provider availability (ADMIN or OPERATIONS)",
  })
  @ApiOkResponse({ type: ProviderAvailabilityResponseDto, isArray: true })
  list(@Param() { providerId }: ProviderIdParamsDto) {
    return this.availability.list(providerId);
  }
  @Post("providers/:providerId/availability")
  @ApiOperation({
    summary: "Create weekly provider availability (ADMIN or OPERATIONS)",
  })
  @ApiCreatedResponse({ type: ProviderAvailabilityResponseDto })
  create(
    @Param() { providerId }: ProviderIdParamsDto,
    @Body() dto: CreateProviderAvailabilityDto,
  ) {
    return this.availability.create(providerId, dto);
  }
  @Get("provider-availability/:id")
  @ApiOperation({ summary: "Get provider availability (ADMIN or OPERATIONS)" })
  @ApiOkResponse({ type: ProviderAvailabilityResponseDto })
  get(@Param() { id }: ResourceIdParamsDto) {
    return this.availability.get(id);
  }
  @Patch("provider-availability/:id")
  @ApiOperation({
    summary: "Update provider availability (ADMIN or OPERATIONS)",
  })
  @ApiOkResponse({ type: ProviderAvailabilityResponseDto })
  update(
  ...
  @Patch("provider-availability/:id/activate")
  @ApiOperation({
    summary: "Activate provider availability (ADMIN or OPERATIONS)",
  })
  @ApiOkResponse({ type: ProviderAvailabilityResponseDto })
  activate(@Param() { id }: ResourceIdParamsDto) {
    return this.availability.activate(id);
  }
  @Patch("provider-availability/:id/deactivate")
  @ApiOperation({
    summary: "Deactivate provider availability (ADMIN or OPERATIONS)",
  })
  @ApiOkResponse({ type: ProviderAvailabilityResponseDto })
  deactivate(@Param() { id }: ResourceIdParamsDto) {
    return this.availability.deactivate(id);
  }
}
