...
import { IsISO4217CurrencyCode, IsInt, IsOptional, Matches, Max, Min } from 'class-validator';
...
export class UpdateProviderServicePriceDto {
  ...
  @ApiProperty({ required: false, description: 'Separate fulfilment fee in integer minor units.' }) @IsOptional() @Type(() => Number) @IsInt() @Min(0) @Max(Number.MAX_SAFE_INTEGER) fulfilmentFeeMinor?: number;
