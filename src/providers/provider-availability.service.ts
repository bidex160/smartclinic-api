...
export class ProviderAvailabilityService {
  ...
  private async rejectOverlap(providerId: string, dayOfWeek: string, startTime: string, endTime: string, serviceId: string | null, locationId: string | null, excludeId?: string): Promise<void> {
    const query = this.availability.createQueryBuilder('availability').where('availability.providerId = :providerId', { providerId }).andWhere('availability.dayOfWeek = :dayOfWeek', { dayOfWeek }).andWhere('availability.providerServiceId IS NOT DISTINCT FROM :serviceId', { serviceId }).andWhere('availability.providerLocationId IS NOT DISTINCT FROM :locationId', { locationId }).andWhere('availability.isActive = true').andWhere('availability.startTime < :endTime', { endTime }).andWhere('availability.endTime > :startTime', { startTime });
