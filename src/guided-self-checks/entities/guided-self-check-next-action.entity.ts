...
import { GuidedSelfCheckAnalysis } from './guided-self-check-analysis.entity';
...
@Entity('guided_self_check_next_actions')
@Index('UQ_gsc_next_action_current', ['guidedSelfCheckId'], { unique: true, where: '"is_current" = true' })
@Index('IDX_gsc_next_action_history', ['guidedSelfCheckId', 'selectedAt'])
export class GuidedSelfCheckNextAction {
  ...
  @Column({ name: 'analysis_id', type: 'uuid', nullable: true }) analysisId!: string | null;
  @ManyToOne(() => GuidedSelfCheckAnalysis, { nullable: true, onDelete: 'RESTRICT' }) @JoinColumn({ name: 'analysis_id' }) analysis!: GuidedSelfCheckAnalysis | null;
