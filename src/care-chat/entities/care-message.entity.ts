...
import { CareMessageAttachment } from './care-message-attachment.entity';
...
@Check('CHK_care_messages_body', '"body" IS NULL OR (char_length("body") BETWEEN 1 AND 4000 AND "body" = btrim("body"))')
export class CareMessage {
  ...
  @Column({ type: 'varchar', length: 4000, nullable: true }) body!: string | null;
  @CreateDateColumn({ name: 'created_at', type: 'timestamptz' }) createdAt!: Date;
  @Column({ name: 'read_at', type: 'timestamptz', nullable: true }) readAt!: Date | null;
  @OneToMany(() => CareMessageAttachment, (attachment) => attachment.careMessage) attachments!: CareMessageAttachment[];
}
