...
export class CareRequestsService {
    ...
    for (let attempt = 0; attempt < MAX_CARE_REQUEST_REFERENCE_ATTEMPTS; attempt += 1) {
      try {
        return await this.requests.manager.transaction(async (manager) => {
          ...
          const geography = deliveryMode === CareDeliveryMode.VIRTUAL ? { countryCode: null, stateOrRegion: null, city: null } : { countryCode: dto.countryCode!, stateOrRegion: dto.stateOrRegion!, city: dto.city! };
          const offering = dto.preferredProviderReference ? await this.eligibility.requireEligible({ careServiceDefinitionId: definition.id, providerReference: dto.preferredProviderReference, ...geography, deliveryMode }, manager) : null;
  ...
  async providerRespond(user: User, reference: string, accept: boolean, reason: string | null) {
    ...
    return this.requests.manager.transaction(async (manager) => {
      ...
      if (accept) {
        await this.eligibility.requireEligible({ careServiceDefinitionId: request.careServiceDefinitionId, providerId: provider.id, countryCode: request.countryCode, stateOrRegion: request.stateOrRegion, city: request.city, deliveryMode: request.deliveryMode }, manager);
        if (request.servicePriceMinor == null || request.serviceCurrency == null) throw new ConflictException('Care Request has no committed service price');
      }
      ...
      if (accept && request.servicePriceMinor === '0' && request.serviceCurrency && !(await manager.getRepository(CareRequestFunding).findOne({ where: { careRequestId: request.id } }))) await manager.getRepository(CareRequestFunding).save({ careRequestId: request.id, amountMinor: '0', currency: request.serviceCurrency, status: CareRequestFundingStatus.SATISFIED_FREE, paidAt: null });
  ...
  async assign(reference: string, actorUserId: string, dto: AssignCareRequestDto) {
    return this.requests.manager.transaction(async (manager) => {
      ...
      request.assignedProviderId = offering.providerId; request.assignedProviderCareServiceId = offering.id; request.servicePriceMinor = offering.selectedDeliveryOption.priceMinor; request.serviceCurrency = offering.selectedDeliveryOption.currency; request.status = CareRequestStatus.AWAITING_PROVIDER_RESPONSE;
  ...
  private validateGeography(deliveryMode: CareDeliveryMode, dto: Pick<CreateCareRequestDto, 'countryCode' | 'stateOrRegion' | 'city'>) { if (deliveryMode !== CareDeliveryMode.VIRTUAL && (!dto.countryCode || !dto.stateOrRegion || !dto.city)) throw new BadRequestException('countryCode, stateOrRegion, and city are required for physical care requests'); }
