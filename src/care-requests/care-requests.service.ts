...
export class CareRequestsService {
    ...
    for (let attempt = 0; attempt < MAX_CARE_REQUEST_REFERENCE_ATTEMPTS; attempt += 1) {
      try {
        return await this.requests.manager.transaction(async (manager) => {
          ...
          const offering = dto.preferredProviderReference ? await this.eligibility.requireEligible({ careServiceDefinitionId: definition.id, providerReference: dto.preferredProviderReference, countryCode: dto.countryCode, stateOrRegion: dto.stateOrRegion, city: dto.city }, manager) : null;
          ...
          const request = await repository.save(repository.create({ reference: generateCareRequestReference(), userId: user.id, patientId: patient.id, careServiceDefinitionId: definition.id, preferredProviderId: offering?.providerId ?? null, preferredProviderCareServiceId: offering?.id ?? null, assignedProviderId: offering?.providerId ?? null, assignedProviderCareServiceId: offering?.id ?? null, countryCode: dto.countryCode, stateOrRegion: dto.stateOrRegion, city: dto.city, notes: dto.notes ?? null, preferredDate: dto.preferredDate ?? null, preferredTime: dto.preferredTime ?? null, contactMethod: dto.contactMethod, status }));
  ...
  async getMine(user: User, reference: string) {
    const patient = await this.requirePatient(user.id);
    const request = await this.readBuilder().where('request.reference = :reference', { reference }).andWhere('request.patientId = :patientId', { patientId: patient.id }).getOne();
  ...
  async getForProvider(user: User, reference: string) {
    const provider = await this.requireOperationalProvider(user);
    const request = await this.readBuilder().where('request.reference = :reference', { reference }).andWhere('request.assignedProviderId = :providerId', { providerId: provider.id }).getOne();
    if (!request) this.notFound();
  ...
  async providerRespond(user: User, reference: string, accept: boolean, reason: string | null) {
    ...
    return this.requests.manager.transaction(async (manager) => {
      ...
      if (accept) await this.eligibility.requireEligible({ careServiceDefinitionId: request.careServiceDefinitionId, providerId: provider.id, countryCode: request.countryCode, stateOrRegion: request.stateOrRegion, city: request.city }, manager);
  ...
  async assign(reference: string, actorUserId: string, dto: AssignCareRequestDto) {
    return this.requests.manager.transaction(async (manager) => {
      ...
      const offering = await this.eligibility.requireEligible({ careServiceDefinitionId: request.careServiceDefinitionId, providerReference: dto.providerReference, countryCode: request.countryCode, stateOrRegion: request.stateOrRegion, city: request.city }, manager);
  ...
  private map(request: CareRequest, providerView = false) { const provider = (value: Provider | null) => value ? { providerReference: value.providerReference, displayName: value.displayName, providerType: value.providerType, location: { city: value.city, stateOrRegion: value.stateOrRegion, countryCode: value.countryCode } } : null; return { reference: request.reference, status: request.status, service: { code: request.careServiceDefinition.code, name: request.careServiceDefinition.name }, geography: { countryCode: request.countryCode, stateOrRegion: request.stateOrRegion, city: request.city }, preferredProvider: provider(request.preferredProvider), assignedProvider: provider(request.assignedProvider), preferredDate: request.preferredDate, preferredTime: request.preferredTime, contactMethod: request.contactMethod, notes: request.notes, createdAt: request.createdAt, updatedAt: request.updatedAt, ...(providerView ? {} : {}) }; }
