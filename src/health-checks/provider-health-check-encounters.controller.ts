...
export class ProviderHealthCheckEncountersController {
  ...
  @Put('measurements') @ApiOperation({ summary: 'Save built-in measurements and snapshotted additional results without completing the encounter' }) @ApiOkResponse({ type: ProviderHealthCheckEncounterResponseDto }) save(@Req() request: { user: User }, @Param() { reference }: BookingReferenceParamsDto, @Body() dto: SaveHealthCheckMeasurementsDto) { return this.encounters.saveMeasurements(request.user, reference, dto); }
  @Post('complete') @HttpCode(HttpStatus.OK) @ApiOperation({ summary: 'Complete an encounter after all result-bearing snapshot requirements are recorded' }) @ApiOkResponse({ type: ProviderHealthCheckEncounterResponseDto }) complete(@Req() request: { user: User }, @Param() { reference }: BookingReferenceParamsDto) { return this.encounters.complete(request.user, reference); }
}
