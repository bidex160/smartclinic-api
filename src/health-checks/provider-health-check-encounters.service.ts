...
export class ProviderHealthCheckEncountersService {
  ...
  async start(user: User, reference: string): Promise<ProviderHealthCheckEncounterResponseDto> {
    ...
    await this.encounters.manager.transaction(async (manager) => {
      ...
      if (![BookingStatus.PROVIDER_ASSIGNED, BookingStatus.SCHEDULED].includes(booking.status)) throw new ConflictException(`Booking in ${booking.status} cannot start a health check encounter`);
      const now = new Date();
  ...
  private toResponse(encounter: HealthCheckEncounter): ProviderHealthCheckEncounterResponseDto { return { bookingReference: encounter.booking.bookingReference, status: encounter.status, startedAt: encounter.startedAt, completedAt: encounter.completedAt, participant: { givenName: encounter.booking.participant.givenName, familyName: encounter.booking.participant.familyName }, healthCheckPackage: { code: encounter.booking.healthCheckPackage.code, name: encounter.booking.healthCheckPackage.name }, fulfilmentMode: { code: encounter.booking.fulfilmentMode.code, name: encounter.booking.fulfilmentMode.name }, measurements: (encounter.measurements ?? []).map((measurement) => ({ code: measurement.code, value: Number(measurement.valueNumeric), secondaryValue: measurement.valueSecondaryNumeric === null ? null : Number(measurement.valueSecondaryNumeric), unit: measurement.unit, recordedAt: measurement.recordedAt })) }; }
