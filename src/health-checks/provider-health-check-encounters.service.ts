...
export class ProviderHealthCheckEncountersService {
  ...
  async get(user: User, reference: string): Promise<ProviderHealthCheckEncounterResponseDto> {
    const provider = await this.currentProvider.resolve(user);
    const encounter = await this.encounters.createQueryBuilder('encounter').innerJoinAndSelect('encounter.booking', 'booking').innerJoinAndSelect('booking.participant', 'participant').innerJoinAndSelect('booking.healthCheckPackage', 'package').innerJoinAndSelect('booking.fulfilmentMode', 'mode').leftJoinAndSelect('booking.providerLocation', 'providerLocation').innerJoinAndSelect('encounter.providerAssignment', 'assignment').leftJoinAndSelect('encounter.measurements', 'measurement').where('booking.bookingReference = :reference', { reference }).andWhere('encounter.providerId = :providerId', { providerId: provider.id }).andWhere('assignment.status = :confirmed', { confirmed: ProviderAssignmentStatus.CONFIRMED }).orderBy('measurement.code', 'ASC').getOne();
  ...
  private toResponse(encounter: HealthCheckEncounter): ProviderHealthCheckEncounterResponseDto { return { bookingReference: encounter.booking.bookingReference, status: encounter.status, startedAt: encounter.startedAt, completedAt: encounter.completedAt, participant: { givenName: encounter.booking.participant.givenName, familyName: encounter.booking.participant.familyName }, healthCheckPackage: { code: encounter.booking.healthCheckPackage.code, name: encounter.booking.healthCheckPackage.name }, fulfilmentMode: { code: encounter.booking.fulfilmentMode.code, name: encounter.booking.fulfilmentMode.name }, confirmedSchedule: encounter.booking.scheduledDate ? { date: encounter.booking.scheduledDate, timeFrom: encounter.booking.scheduledTimeFrom!, timeTo: encounter.booking.scheduledTimeTo!, timezone: encounter.booking.scheduledTimezone!, providerLocationName: encounter.booking.providerLocation?.name ?? null } : null, measurements: (encounter.measurements ?? []).map((measurement) => ({ code: measurement.code, value: Number(measurement.valueNumeric), secondaryValue: measurement.valueSecondaryNumeric === null ? null : Number(measurement.valueSecondaryNumeric), unit: measurement.unit, recordedAt: measurement.recordedAt })) }; }
