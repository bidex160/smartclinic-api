...
export class HealthCheckConfigurationService {
  ...
  async quote(user:User,dto: HealthCheckConfigurationQuoteDto) {
    ...
    const base = BigInt(service.priceMinor);\n    // Travel pricing for Home Visit is determined during discovery from the patient's geography.\n    // Configuration quotes do not currently receive that geography, so never invent or trust a client travel fee here.\n    // The booking/payment path therefore remains authoritative for the persisted quoted fulfilment fee.\n    const fee = BigInt(service.fulfilmentFeeMinor ?? '0');
