...
export class HealthCheckConfigurationService {
  ...
  async quote(user:User,dto: HealthCheckConfigurationQuoteDto) {
    ...
    const base = BigInt(service.priceMinor);
    let travelFeeMinor = 0;
    if (service.fulfilmentMode.code === 'HOME_VISIT') {
      if (!dto.countryCode || !dto.stateOrRegion || !dto.city) throw new BadRequestException('Country, state or region, and city are required to quote a Home Visit');
      let areas: any[] = [];
      try { areas=await this.services.manager.query(`SELECT travel_fee_minor FROM provider_service_areas WHERE provider_service_id=$1 AND is_active=true AND country_code=$2 AND LOWER(TRIM(state_or_region))=LOWER(TRIM($3)) AND (city IS NULL OR LOWER(TRIM(city))=LOWER(TRIM($4))) AND (postal_code IS NULL OR LOWER(TRIM(postal_code))=LOWER(TRIM($5))) ORDER BY priority ASC,travel_fee_minor ASC`,[service.id,dto.countryCode.toUpperCase(),dto.stateOrRegion,dto.city,dto.postalCode??'']); }
      catch { areas=await this.services.manager.query(`SELECT 0 AS travel_fee_minor FROM provider_service_areas WHERE provider_service_id=$1 AND is_active=true AND country_code=$2 AND LOWER(TRIM(state_or_region))=LOWER(TRIM($3)) AND (city IS NULL OR LOWER(TRIM(city))=LOWER(TRIM($4))) AND (postal_code IS NULL OR LOWER(TRIM(postal_code))=LOWER(TRIM($5)))`,[service.id,dto.countryCode.toUpperCase(),dto.stateOrRegion,dto.city,dto.postalCode??'']); }
      if (!areas.length) throw new ConflictException('Provider no longer covers this Home Visit area');
      travelFeeMinor=Number(areas[0].travel_fee_minor??0);
    }
    const fee = BigInt(service.fulfilmentFeeMinor ?? '0') + BigInt(travelFeeMinor);
