...
export class HealthCheckConfigurationService {
  ...
  async discover(dto:HealthCheckOfferingDiscoveryDto){const [pkg,mode]=await Promise.all([this.packages.findOne({where:{code:dto.packageCode,isActive:true}}),this.modes.findOne({where:{code:dto.fulfilmentModeCode,isActive:true}})]);if(!pkg||!mode)throw new NotFoundException('Health Check package or fulfilment mode not found');const end=deriveAppointmentEndTime(dto.preferredTime,pkg.estimatedDurationMinutes??0);if(!end)throw new BadRequestException('Health Check package duration is invalid');const eligible=await this.capabilities.findEligibleProviders(pkg.id,mode.id,{requestedDate:dto.preferredDate,requestedStartTime:dto.preferredTime,requestedEndTime:end,requestedTimezone:dto.timezone,visitAddress:{countryCode:dto.countryCode,stateOrRegion:dto.stateOrRegion,city:dto.city,postalCode:dto.postalCode??null,addressLine1:'Discovery context'}});const ids=eligible.map((x)=>x.id);if(!ids.length)return{items:[],page:dto.page,limit:dto.limit,total:0,totalPages:0};const rows=await this.services.find({where:{id:In(ids)},relations:{provider:true,healthCheckPackage:{contents:true,addonAvailability:{addon:true}},fulfilmentMode:true,locationLinks:{providerLocation:true},addons:{addon:true}}});const projected=rows.map((s)=>({providerReference:s.provider.providerReference,providerName:s.provider.displayName,packageCode:s.healthCheckPackage.code,basePackagePriceMinor:Number(s.priceMinor),currency:s.currency,fulfilmentMode:{code:s.fulfilmentMode.code,name:s.fulfilmentMode.name,fulfilmentFeeMinor:Number(s.fulfilmentFeeMinor??0)},locations:s.fulfilmentMode.code==='PROVIDER_LOCATION'?(s.locationLinks??[]).filter((x)=>x.providerLocation.isActive).map((x)=>({reference:x.providerLocation.locationReference,name:x.providerLocation.name,addressLine1:x.providerLocation.addressLine1,addressLine2:x.providerLocation.addressLine2,city:x.providerLocation.city,stateOrRegion:x.providerLocation.state,postalCode:x.providerLocation.postalCode,countryCode:x.providerLocation.countryCode})):[],addons:(s.addons??[]).filter((x)=>x.isActive&&x.addon.isActive&&x.currency===s.currency&&(s.healthCheckPackage.addonAvailability??[]).some((a)=>a.isActive&&a.addonId===x.addonId)&&!(s.healthCheckPackage.contents??[]).some((c)=>c.isActive&&c.code===x.addon.code)).map((x)=>({code:x.addon.code,name:x.addon.name,category:x.addon.category,priceMinor:Number(x.priceMinor),currency:x.currency}))}));const start=(dto.page-1)*dto.limit;return{items:projected.slice(start,start+dto.limit),page:dto.page,limit:dto.limit,total:projected.length,totalPages:Math.ceil(projected.length/dto.limit)};}
