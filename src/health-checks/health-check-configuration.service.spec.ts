...
describe('HealthCheckConfigurationService', () => {
  ...
  it('uses area-only geography for PROVIDER_LOCATION discovery without fabricating a street address', async () => { const capabilities = { findEligibleProviders: jest.fn().mockResolvedValue([]) }; const subject = new HealthCheckConfigurationService({} as never, {} as never, {} as never, {} as never, { findOne: jest.fn().mockResolvedValue({ id: 'package', estimatedDurationMinutes: 30 }) } as never, { findOne: jest.fn().mockResolvedValue({ id: 'mode' }) } as never, capabilities as never); await subject.discover({ packageCode: 'ESSENTIAL', fulfilmentModeCode: 'PROVIDER_LOCATION', preferredDate: '2026-09-10', preferredTime: '09:00', timezone: 'Africa/Lagos', countryCode: 'NG', stateOrRegion: 'Lagos', city: 'Ikeja', page: 1, limit: 20 }); expect(capabilities.findEligibleProviders).toHaveBeenCalledWith('package', 'mode', expect.objectContaining({ visitAddress: { countryCode: 'NG', stateOrRegion: 'Lagos', city: 'Ikeja', postalCode: null } })); });
});
