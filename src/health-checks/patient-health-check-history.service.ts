...
export class PatientHealthCheckHistoryService {
  ...
  async list(user: User, query: PatientHealthCheckHistoryQueryDto): Promise<PatientHealthCheckHistoryResponseDto> {
    ...
    const builder = this.bookings.createQueryBuilder('booking').innerJoinAndSelect('booking.healthCheckPackage', 'package').innerJoinAndSelect('booking.fulfilmentMode', 'fulfilmentMode').leftJoinAndSelect('booking.providerLocation', 'providerLocation').where('booking.participantPatientId = :patientId', { patientId: patient.id });
  ...
  private map(booking: Booking, encounter: HealthCheckEncounter | null, providerDisplayName: string | null): PatientHealthCheckHistoryItemDto { return { bookingReference: booking.bookingReference, bookingStatus: booking.status, createdAt: booking.createdAt, updatedAt: booking.updatedAt, healthCheckPackage: { code: booking.healthCheckPackage.code, name: booking.healthCheckPackage.name }, fulfilmentMode: { code: booking.fulfilmentMode.code, name: booking.fulfilmentMode.name }, preferredDate: booking.preferredDate, preferredTimeFrom: booking.preferredTimeWindowStart, preferredTimeTo: booking.preferredTimeWindowEnd, preferredTimezone: booking.preferredTimezone, confirmedSchedule: booking.scheduledDate ? { date: booking.scheduledDate, timeFrom: booking.scheduledTimeFrom!, timeTo: booking.scheduledTimeTo!, timezone: booking.scheduledTimezone!, providerLocationName: booking.providerLocation?.name ?? null } : null, providerDisplayName, encounterStatus: encounter?.status ?? null, startedAt: encounter?.startedAt ?? null, completedAt: encounter?.completedAt ?? null, hasCompletedResult: encounter?.status === HealthCheckEncounterStatus.COMPLETED }; }
