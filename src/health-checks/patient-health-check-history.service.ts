...
export class PatientHealthCheckHistoryService {
  ...
  async list(user: User, query: PatientHealthCheckHistoryQueryDto): Promise<PatientHealthCheckHistoryResponseDto> {
    const builder = this.bookings.createQueryBuilder('booking').innerJoinAndSelect('booking.healthCheckPackage', 'package').innerJoinAndSelect('booking.fulfilmentMode', 'fulfilmentMode').innerJoinAndSelect('booking.participant', 'participant').leftJoinAndSelect('booking.providerLocation', 'providerLocation').leftJoinAndSelect('booking.visitAddress', 'visitAddress').where('booking.bookerUserId = :bookerUserId', { bookerUserId: user.id });
  ...
  async get(user: User, reference: string): Promise<PatientHealthCheckDetailResponseDto> {
    const booking = await this.bookings.findOne({ where: { bookingReference: reference, bookerUserId: user.id }, relations: { healthCheckPackage: true, fulfilmentMode: true, participant: true, providerLocation: true, visitAddress: true } });
  ...
  private commercial(booking:Booking):Record<string,unknown>|null{const x=booking.commercialConfigurationSnapshot as any;if(!x)return null;return{package:x.package??null,provider:x.provider??null,providerLocation:x.providerLocation??null,fulfilmentMode:x.fulfilmentMode??null,includedContents:Array.isArray(x.includedContents)?x.includedContents:[],selectedAddons:Array.isArray(x.selectedAddons)?x.selectedAddons:[],pricing:{basePackagePriceMinor:Number(booking.basePackagePriceMinor??0),clinicalAddonsTotalMinor:Number(booking.clinicalAddonsTotalMinor??0),fulfilmentFeeMinor:Number(booking.fulfilmentFeeMinor??0),totalMinor:booking.quotedAmount?Math.round(Number(booking.quotedAmount)*100):null,currency:booking.currency}};}
}
